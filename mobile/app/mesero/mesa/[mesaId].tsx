...
import { Alert, SafeAreaView, ScrollView, Text, TouchableOpacity, View, Modal, FlatList, Image, Dimensions, TextInput } from 'react-native';
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          borderRadius: 8,
          borderWidth: 2,
          borderColor: cantidad > 0 ? '#3b82f6' : (dark ? '#374151' : '#E5E7EB'),
      ...
      >
        ...
        {/* Precio */}
        {p.precio != null && (
          <Text style={{ fontSize: 14, fontWeight: '700', color: '#3b82f6', textAlign: 'center' }}>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: bg, paddingBottom: footerHeight + 12 }}>
      ...
      <Modal visible={showSuccessGif} animationType="fade" transparent>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.35)' }}>
          {/* Render GIF without a white/backplate — user supplied different gifs per theme */}
          <View style={{ width: 180, height: 180, alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' }}>
            <Image
              source={dark ? require('../../../assets/dark_check.gif') : require('../../../assets/check.gif')}
      ...
      {tieneComandas && comandas.map((comanda, index) => {
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            borderRadius: 12,
            padding: comandaContraida ? 10 : 12,
            backgroundColor: dark ? '#0b1220' : '#FFF7ED',
            borderWidth: 1,
            borderColor: dark ? '#374151' : '#F59E0B',
            flexDirection: 'row',
        ...
        >
          {/* pushpin visual */}
          <View style={{ width: comandaContraida ? 24 : 36, height: comandaContraida ? 24 : 36, alignItems: 'center', justifyContent: 'center' }}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 6 }}>
                  {comanda.pedidos?.slice(0, 5).map((p: any) => (
                    <View key={p.id} style={{ marginRight: 8, padding: 8, borderRadius: 8, backgroundColor: dark ? '#0b1220' : 'rgba(245, 158, 11, 0.08)', borderWidth: 1, borderColor: dark ? '#374151' : 'rgba(245,158,11,0.22)' }}>
                      <Text style={{ color: dark ? '#D1D5DB' : '#92400E', fontWeight: '700', fontSize: 12 }}>{p.cantidad}x {p.producto?.nombre || 'Producto'}</Text>
                      <Text style={{ color: dark ? '#D1D5DB' : '#92400E', fontWeight: '600', fontSize: 11 }}>Bs {Number(p.precioUnitario || p.producto?.precio || 0).toFixed(2)}</Text>
                      {p.notas ? (
      ...
      {tieneComandas && !comandaContraida && (
        <View style={{ paddingHorizontal: 16, marginTop: 6, zIndex: 50, elevation: 8, pointerEvents: 'auto' }}>
          ...
          <TouchableOpacity onPress={() => { console.log('[MesaProductos] Crear nueva comanda pressed (user.tipo)', user?.tipo); setShowProductsMode('new'); setSelectedComanda(null); resetCantidades(); setComandaContraida(true); }} style={{ paddingVertical: 12, paddingHorizontal: 16, marginBottom: 12, borderRadius: 10, backgroundColor: '#f3f4f6', borderWidth: 1, borderColor: '#e5e7eb', alignItems: 'center' }}>
          ...
          {([ 
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                marginRight: 8,
                borderRadius: 8,
                backgroundColor: viewMode === b.key ? '#DBEAFE' : '#F3F4F6',
                borderWidth: 1,
                borderColor: viewMode === b.key ? '#60A5FA' : '#E5E7EB',
              }}
            >
              <Text style={{ color: viewMode === b.key ? '#1D4ED8' : '#374151', fontWeight: '600' }}>{b.label}</Text>
            </TouchableOpacity>
            ))}
          </View>
        </View>
      ...
      {showProductsGrid ? (
        ...
        <Animated.View style={{ padding: 20, alignItems: 'center', opacity: showProductsAnim, transform: [{ translateY: showProductsAnim.interpolate({ inputRange: [0, 1], outputRange: [6, 0] }) }] }}>
          <Text style={{ color: muted }}>Productos ocultos — selecciona “➕ Agregar productos” o “➕ Crear nueva comanda” para ver la carta</Text>
        </Animated.View>
      )}
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          ...
          >
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ fontSize: 14, color: muted, marginTop: 2 }}>
                          {cantidad} x Bs {Number(producto.precio).toFixed(2)}
                        </Text>
                        {/* Nota editable */}
                        <TextInput
            ...
            <View style={{ flexDirection: 'row', gap: 12, paddingHorizontal: 20 }}>
              <TouchableOpacity
                onPress={() => setMostrarPedido(false)}
                style={{
                  flex: 1,
                  paddingVertical: 14,
                  borderRadius: 12,
              ...
              >
                <Text style={{ color: 'white', fontWeight: '700', fontSize: 16 }}>
                  {loading ? 'Enviando...' : `Enviar Pedido (${totalItems})`}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
            ...
            <ScrollView style={{ paddingHorizontal: 20, maxHeight: 380 }}>
              {selectedComanda?.pedidos?.map((p: any) => (
                <View key={p.id} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: dark ? '#374151' : '#E5E7EB' }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: fg, fontWeight: '700' }}>{p.cantidad} x {p.producto?.nombre}</Text>
                    <Text style={{ color: muted, fontSize: 12 }}>{p.notas}</Text>
                  </View>
                  <Text style={{ color: '#3b82f6', fontWeight: '700' }}>Bs {Number(p.precioUnitario || p.producto?.precio || 0).toFixed(2)}</Text>
                </View>
              ))}
              ...
              <View style={{ marginTop: 12, padding: 12, borderRadius: 8, backgroundColor: dark ? '#0b1220' : '#FEF3C7', borderWidth: 1, borderColor: dark ? '#374151' : '#F59E0B' }}>
                <Text style={{ fontWeight: '700', color: dark ? 'white' : '#92400E' }}>
                  Total: Bs {Number((selectedComanda?.pedidos || []).reduce((s: number, x: any) => s + Number(x.subtotal || (x.precioUnitario || x.producto?.precio || 0) * x.cantidad), 0)).toFixed(2)}
                </Text>
              </View>
            </ScrollView>
      ...
      <Modal visible={showPaymentModal} animationType="slide" transparent>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: bg, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20 }}>
            ...
            <Text style={{ color: muted, fontSize: 12, marginBottom: 12 }}>Total: Bs {Number((selectedComanda?.pedidos || []).reduce((s: number, x: any) => s + Number(x.subtotal || (x.precioUnitario || x.producto?.precio || 0) * x.cantidad), 0)).toFixed(2)}</Text>

            {/* Opciones de pago */}
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
              {(['efectivo','qr','mixto'] as any[]).map((m) => (
                <TouchableOpacity key={m} onPress={() => setPagoMetodo(m)} style={{ paddingVertical: 10, paddingHorizontal: 14, borderRadius: 8, borderWidth: 1, borderColor: pagoMetodo === m ? '#60A5FA' : (dark ? '#374151' : '#E5E7EB'), backgroundColor: pagoMetodo === m ? '#DBEAFE' : (dark ? '#0b1220' : '#f8fafc') }}>
                  <Text style={{ fontWeight: '700', color: pagoMetodo === m ? '#1D4ED8' : muted }}>{m === 'efectivo' ? 'Efectivo' : m === 'qr' ? 'QR' : 'Mixto'}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            {(pagoMetodo === 'qr' || pagoMetodo === 'mixto') && (
              <View style={{ marginBottom: 12 }}>
                <Text style={{ color: muted, fontSize: 12, marginBottom: 8 }}>Comprobante (QR)</Text>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  <TouchableOpacity onPress={async () => {
                    ...
                    if (!result.canceled && result.assets[0]) setComprobanteImagen(`data:image/jpeg;base64,${result.assets[0].base64}`);
                  ...
                  <TouchableOpacity onPress={async () => {
                    ...
                    <Text style={{ color: 'white', fontWeight: '700' }}>🖼️ Galería</Text>
                  </TouchableOpacity>
                </View>

                {comprobanteImagen && (
                  <View style={{ marginTop: 12 }}>
                    <Image source={{ uri: comprobanteImagen }} style={{ width: '100%', height: 180, borderRadius: 8 }} resizeMode='contain' />
                    <TouchableOpacity onPress={() => setComprobanteImagen(null)} style={{ marginTop: 8, padding: 8, backgroundColor: '#ef4444', borderRadius: 8, alignItems: 'center' }}>
