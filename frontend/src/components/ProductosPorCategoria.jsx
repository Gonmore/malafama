...
const ProductosPorCategoria = ({ onProductoSeleccionado, localId }) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-3 text-slate-400">Cargando productos...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-red-900/20 border border-red-200 rounded-lg p-4 text-red-800">
        {error}
      </div>
    );
  }
  ...
  if (categorias.length === 0) {
    return (
      <div className="bg-yellow-900/20 border border-yellow-200 rounded-lg p-4 text-yellow-800">
      ...
      <div className="border-b border-slate-700 bg-slate-800">
        <nav className="flex space-x-1 overflow-x-auto px-4" aria-label="Categorías">
          {categorias.map((cat) => (
            ...
            >
              {cat.categoria}
              <span className="ml-2 px-2 py-0.5 rounded-full text-xs bg-slate-800">
                {cat.total}
              </span>
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {productosCategoria?.productos.map((producto) => (
            <button
              ...
              className="bg-slate-800 rounded-lg shadow hover:shadow-lg transition-shadow p-4 text-left border border-slate-700 hover:border-blue-400"
            >
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                {/* Estado */}
                {!producto.activo && (
                  <span className="text-xs px-2 py-1 rounded bg-red-900/30 text-red-800">
                    No disponible
                  </span>
                )}
              </div>
