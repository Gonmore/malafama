      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
        <div className={`${darkMode ? 'bg-gray-900' : 'bg-white'} rounded-3xl p-8 shadow-2xl`}>
          <div className="text-center">
            ...
            <p className={`mt-4 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Cargando reporte...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-50 p-4 overflow-y-auto">
      <div className={`${darkMode ? 'bg-gray-900' : 'bg-white'} rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-4xl max-h-[95vh] flex flex-col my-4`}>
                          ...
                          <div className={`mb-3 p-3 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                              {comanda.formaPago === 'efectivo' && (
                                <span className={`px-3 py-1 rounded-full text-sm font-semibold ${darkMode ? 'bg-green-900/30 text-green-300' : 'bg-green-100 text-green-700'}`}>
                                  💵 Efectivo: {formatearMoneda(comanda.total)}
                                </span>
                              )}
                              {comanda.formaPago === 'qr' && (
                                <>
                                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${darkMode ? 'bg-purple-900/30 text-purple-300' : 'bg-purple-100 text-purple-700'}`}>
        ...
        {/* Footer */}
        <div className={`p-6 border-t ${darkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'} flex-shrink-0`}>
          <button
            onClick={onClose}
