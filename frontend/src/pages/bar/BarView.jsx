...
export default function BarView() {
  ...
  const [modoVista, setModoVista] = useState(() => localStorage.getItem('bar_modo_vista') || 'por-pedido'); // por-pedido | por-pedido-compacto | por-producto | por-mesa
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('bar_dark_mode') === 'true');
  ...
  const [productosSeleccionados, setProductosSeleccionados] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem(`bar_prod_filter_${user?.id || 'anon'}`) || '[]'));
    } catch (e) {
      return new Set();
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem('bar_dark_mode', darkMode);
  }, [darkMode]);

  useEffect(() => {
    localStorage.setItem(`bar_prod_filter_${user?.id || 'anon'}`, JSON.stringify(Array.from(productosSeleccionados)));
  ...
  const cargarCatalogoProductos = async () => {
    try {
      ...
      const productosBebida = productos.filter((producto) => String(producto?.tipo || '').toLowerCase() === 'bebida');
      setCatalogoProductos(productosBebida);
    } catch (error) {
          ...
          return (
            <div key={producto} className="bg-blue-50/30 rounded-lg shadow-md border-2 border-blue-300 overflow-hidden">
              <div className="flex">
                ...
                <div className="w-[30%] bg-blue-500 text-white p-3 flex items-center justify-center">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-center mb-1">
                          <span className={`text-xs font-bold ${estaListo ? 'text-gray-500' : 'text-blue-600'}`}>
                            x{pedido.cantidad}
                          </span>
          ...
          return (
            <div key={producto} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
              ...
              <div className="bg-blue-500 text-white px-6 py-4">
                ...
                <p className="text-sm text-blue-100">{pedidosProducto.length} pedido(s) pendiente(s)</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start mb-2">
                        {/* Cantidad con x */}
                        <div>
                          <span className={`text-2xl font-bold ${estaListo ? 'text-gray-500' : 'text-blue-600'}`}>
                            x{pedido.cantidad}
                          </span>
                        </div>
            ...
            <div className="mb-2">
              <div className="flex items-center justify-between">
                  <h4 className={`font-bold ${titleSize} ${estaListo ? 'text-gray-500' : darkMode ? 'text-gray-200' : 'text-gray-800'} mb-0.5 leading-tight`}> 
                    <span className={`${qtySize} font-bold ${estaListo ? 'text-gray-500' : darkMode ? 'text-cyan-400' : 'text-blue-600'} inline-block mr-2 align-middle`}>x{pedido.cantidad}</span>
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      {/* Navbar */}
      <Navbar roleLabel="Bar" pedidosCount={pedidosPendientesFiltrados.length} />
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  ...
                  localStorage.setItem('bar_modo_vista', e.target.value);
                }}
                className={`appearance-none text-sm border rounded-md pl-3 pr-8 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer transition-colors ${
                  darkMode
      ...
      {filtroProductosAbierto && (
        <div className={`border-b ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-blue-50 border-blue-100'}`}>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-sm border transition-colors ${
                  productosSeleccionados.size === 0
                    ? darkMode ? 'bg-blue-500 text-white border-blue-400' : 'bg-blue-600 text-white border-blue-600'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          <div className="text-center py-12">
            <div className={`animate-spin rounded-full h-12 w-12 border-b-2 mx-auto ${darkMode ? 'border-blue-400' : 'border-blue-600'}`}></div>
