...
async function resolveAllowedLocalIds(req) {
  ...
  // Empleados (atencion/cocina/bar/proveedor) van siempre por su local
...
function buildLocalWhere(allowedLocalIds, requestedLocalId) {
  ...
  if (requestedLocalId) {
    if (!allowedLocalIds.includes(requestedLocalId)) {
      const err = new Error('No tienes permisos para acceder a este local');
      err.status = 403;
      throw err;
    }
    return { localId: requestedLocalId };
  }
...
function assertLocalIdAllowed(allowedLocalIds, localId) {
  ...
  if (!localId) return;
  if (!allowedLocalIds.includes(localId)) {
    const err = new Error('No tienes permisos para acceder a este local');
    err.status = 403;
    throw err;
  }
